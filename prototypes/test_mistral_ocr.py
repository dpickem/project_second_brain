...
def run_basic_ocr(client: Mistral, pdf_path: str, include_images: bool = False) -> Optional[OCRResponse]:
    """
...
def run_annotated_ocr(
    ...
    try:
        ...
        if "unexpected keyword argument" in str(e):
            ...
            print("   To enable annotations, upgrade with: pip install --upgrade mistralai")
    ...
    for page in response_dict.get("pages", []):
        for img in page.get("images", []):
            if "image_base64" in img:
                img["image_base64"] = "[BASE64_DATA_REMOVED - use markdown file with --include-images]"
...
Examples:
    ...
    parser.add_argument(
        "--json-only",
        action="store_true",
        help="Print raw JSON response and exit."
    )
